...
def findQR(turn_direction = None, dist_target = 200):
  ...
  while QR_FOUND[0] == False:
    ...
    time.sleep(1)
    QR_FOUND = angleAdjust(dist_target)
    if abs(orient) >= 45 and TURN_BACK == False:
      turn_inc = -turn_inc
      TURN_BACK = True
      orient = 0
    elif MOVE_FORWARD == True:
      if abs(orient) >= 45:
        Motors.moveDistance(40)
        MOVE_FORWARD = False
        orient = 0
        time.sleep(2)
    elif TURN_BACK == True:
...
while True:
  ...
  elif dist_target <= min_obj_Ultradist + 15:
    Motors.stop()
    Motors.moveDistance(dist_target)
    time.sleep(2)
    print("we're going dbah")
    HEAD_HOME = True
  ...
  if ultra_dist > min_obj_Ultradist: #Enter if there is no object infront of Jankanator
    if AVOIDING: # Check if we are in the avoiding stage
      ...
      if INITIAL_EVADE:
        ...
        time.sleep(1) # adjustable when optimising -> could make a min sleep function that determines time needed to move based on mtr_speed
