...
#Begin algorithm
while True:
  while SCANNING: # Scanning for QR code
    image = Robot.getImageCapture(IMRES.R1920x1088)
    image = image.astype(np.uint8)
    barcodes = pyzbar.decode(image)
    if barcodes == []:
      Motors.turnDegrees(20)
      time.sleep(0.7)
      continue 
    ...
    cv2.putText(image, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX,
      0.5, (0, 0, 255), 2)
    # Calculate distance
    dist_target = QRdistance(x)
    SCANNING = False # Found a QR code therefore skat
  ...
  target_check_time = timer() # get current time
  time_moving_to_target_check = target_check_time - target_time0 # find difference from time it started moving towards the target
